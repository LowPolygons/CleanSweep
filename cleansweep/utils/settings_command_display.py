...
def SettingsCommandDisplay(option: SettingsVariant, maybe_settings: Optional[UserSettings] = None) -> None:
    ...
    print(f"Override Files (Keep -> Delete):")
    print(f"16 - ..with extension: {maybe_settings.override_files_with_extension}")
    print(f"17 - ..name contains: {maybe_settings.override_file_names_containing}")
    print(f"18 - ..name starts with: {maybe_settings.override_file_names_starting_with}")
    print(f"19 - ..directory contains: {maybe_settings.override_files_whos_directory_contains}")
    print(f"20 - ..larger than: {maybe_settings.override_files_larger_than} bytes\n")
    print(f"21 - Consider Access Date when Filtering: {maybe_settings.consider_access_date_when_filtering}")
